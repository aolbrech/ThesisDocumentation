...
%The vast majority of the physical objects produced during particle collisions can be reconstructed using the collected energy deposits, however neutrinos are the exception to the rule. Neutrinos are weakly interacting particles and will therefore traverse the entire detector without leaving behind a single energy deposit. Hence neutrinos escape detection and cannot be reconstructed in a straightforward manner. 
\\
...
%The solution for obtaining an accurate neutrino reconstruction, primordial for describing $t\bar{t}$ events, lies to a great extent in the geometric structure of the particle detector. The CMS detector is developed to be as hermeticcaly closed as possible in order to ensure that the sole particles leaving the detector without interaction are neutrinos, and perhaps other hypothetical neutral weakly interacting particles. As such the high reconsctruction efficiency of the particle-flow algorithm can be exploited by using the total transverse momentum of all observed final-state particles to define the missing transverse momentum.
\begin{equation}
 \vecmet = - \sum \vec{\pT}
\end{equation}
