\chapter{Event topology reconstruction} \label{chp:labelTitle}
...
The overall event count obtained in simulation, however, is not affected by the pileup reweighting since this correction factors is on average close to $1$ (\textbf{averages out to unity}).
The importance of this correction can be visualized \textbf{from/in} Figure~\ref{fig::PUInfl}, which contains the primary vertex multiplicity with and without the reweighting applied.
...
%does not share this characteristic of an average equal to 1
%With an average of about 0.87 $\pm$  (\textit{Get the uncertainty from up/down}), 
...
\begin{figure}[h!t]
 ...
 \caption{Influence of pu reweighting (FILL) -- first explain all scalefactors and only then show plots!!! (leptonSF and b-tag is applied ..)\\ Still need to change the x-axis to go from -0.5 to 35.5!} \label{fig::PUInfl}
\end{figure}
...
% \caption{Influence of lepton SF (FILL) (PU and b-tag applied ..) \\ Limit the range!!} \label{fig::LeptonSFInfl}
%\end{figure}
