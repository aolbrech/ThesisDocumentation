...
\\
\\
The last but definitely not least important calibration that needs to be applied corrects for the slightly different efficiency of the b-jet identification algorithm in data and simulation.
This aspect has been studied in great detail and has resulted in the development of b-tag efficiency scale-factors, analytic functions depending on the transverse momentum of the jets, to account for these discrepancies~\cite{BTagSF}.
%since detailed studies have indicated that the b-jet identification efficiency and the misidentification are slightly different for data than for simulated events.
%Since here the Combined Secondary Vertex b-jet identification algorithm is used at the Tight working point, the minor discrepancies are enlarged such that the overall correction factor obtained becomes rather significant.
\\
These scale-factors are used to correct the measured jet-dependent efficiencies in simulation in order to obtain an overall event efficiency as expected in data, which is then compared to the corresponding event efficiency in simulation. %\textit{The in this way determined event-weight is thus analysis-specific since it will only use events passing the selection requirements.}
%These scale-factors are used to determine where the overall event efficiency for data is obtained from simulation by taking into account the corresponding scale-factor.
%Since they are jet-specific 
\\
%However, this area has been studied in great detail and b-tag efficiency scale-factors have been determined in order to correct the simulated events for the observed disagreement~\cite{BTagSF}.
%The calculated scale-factors take the form of an analytic function depending on the transverse momentum $\pT$ of each individual jet. % and will therefore adjust every jet present in the simulated event such that it corresponds with the data expectation.
These allow to adjust every jet present in the simulated event such that it corresponds with the data expectation such that the overall event outcome between actual simulation and the data-corrected simulation events can be compared.
%Hence the event-weight is calculated by first adjusting each jet present in the simulated event to correspond with the data expectation and afterwards comparing the overall event outcome between actual simulation and data-corrected simulation events.
The benefit of this approach is that the obtained event-weight only uses events passing the selection requirements.
the analysis-specificness of the obtained event-weight since only the events passing the selection requirements will be used.
\\
\\
The b-tag event-weight determination starts by computing the the probability $\varepsilon_{f}$ for a jet of flavour $f$ to identified as a b-jet using the simulated semi-leptonic $\ttbar$ events.
%At first the probability $\varepsilon_{f}$ for a jet of flavour $f$ to identified as a b-jet has to be determined from the simulated semi-leptonic $\ttbar$ events.
From this an overall event probability $P_{sim}$, see Equation (\ref{eq::PSim}), is obtained by multiplying the probabilities of all jets\footnote{It is important to note that in Equation (\ref{eq::PSim}) the labels \textit{tagged} and \textit{non-tagged} refer to whether the considered jet fullfilled the applied b-tag requirement and not whether it has been labelled as one of the two b-jet or light jet candidates in the event selection.}.
The corresponding probability as expected in data, $P_{data}$ in Equation(\ref{eq::PData}), is determined similarly by scaling the calculated efficiencies in order to correspond with those expected in data.
%One of the more significant calibrations, since it does not average out to unity and thus influences the event count, is associated with the choice of using the b-jet identification algorithm at the Tight working point. 
...
\begin{eqnarray} 
 ...
 P_{data} & = & \prod_{i = tagged} SF_{i} ~ \varepsilon_{i} \prod_{j = non-tagged} (1- SF_{i} ~ \varepsilon_{j}) \label{eq::PData}
\end{eqnarray}
The overall event-weight then follows in a straight-forward manner by dividing the data probability with the simulation probability:
\begin{equation}
...
\begin{figure}[h!t]
 ...
 \caption{Influence of pu reweighting (FILL) -- first explain all scalefactors and only then show plots!!! (leptonSF and b-tag is applied ..)} \label{fig::PUInfluence}
\end{figure}
