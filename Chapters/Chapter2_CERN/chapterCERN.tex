...
The benefit of using an iterative process is that during the first iteration the easiest to find tracks are identified such that their associated hits can be removed from the list in order to reduce combinatorial complexity for the following iterations.
...
\begin{myindentpar}
  \begin{description}
    ...
    This module of the CTF algorithm is basically a Kalman Filter which proceeds iterative from the seed layer until the outermost tracker layer is reached. % taking into account the effects of multiple scattering and energy loss. 
