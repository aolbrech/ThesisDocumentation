...
\end{eqnarray}
where $V_L$, $V_R$, $g_L$ and $g_R$ are the complex coupling coefficients for the vector and tensor interaction, respectively. Assuming CP conservation, which has been verified experimentally by the measurement of the top-mass difference~\cite{}, obliges these coupling coefficients to be real~\footnote{The only CP violation is in the CKM matrix.}.
\\

In the Standard Model limit both tensor couplings and the right-handed vector coupling vanish at tree-level, corresponding with the observed lefthandedness of the weak interaction.
...
The main difficulty arising when measuring the anomalous coupling coefficients in the Wtb interaction vertex is the absence of a clear and direct visualization.
This because no single variable exist that summarizes the influence of altering the Wtb vertex, only the effect on distinct kinematic properties can be studied. % in order to understand the relative importance.
...
\\
Hence this allowed to perform a phenomenological survey on the importance of each of the distinct coupling coefficients, which indicated that the Wtb interaction is not influenced in case only a single coupling constant is varied since such a contribution gets cancelled out. Even more important, this survey also pointed out that the strongest dependence is obtained for the right-handed tensor coupling $g_R$ because this is the only one with a non-suppressed interference term of the form $V_L g_R^{*}$ dominating over the quadratic terms.
As a result, the analysis discussed in this thesis will focus on the measurement of this right-handed tensor coupling while restraining all other ones to their Standard Model predictions.
\\

The next part of this section will now be devoted to the specific case of the right-handed tensor coupling and the expected influence of this coupling coefficient on the $\theta^{*}$ distribution.
\\
\textit{Give the formula for the case when only VL and gR are different from zero + explain this b-mass importance! (and maybe top mass influence)}
\\

To finalize the discussion of the anomalous coupling coefficients the experimental limits obtained up to now will briefly be discussed.
\\
\textit{Results were obtained from different measurements! (See paper of aguilar-saavedra)}
