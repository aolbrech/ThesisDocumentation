...
Besides the production cross-section is also the top quark mass an extremely important property which might give some clues about physics beyond the Standard Model. \textit{Why is this top mass measurement so important, is it only for the link with the BEH boson mass?}
\\
...
A common strategy adopted in spin correlations studies is in stead of measuring the spin correlation strenght to determine the degree of spin correlation relative to the SM prediciton $f_{SM}$. 
...
The most precise results have been obtained by ATLAS in the dilepton channel by exploiting the azimuthal angle between the two leptons~\cite{AtlasSpinCorr}:
\begin{equation}
 f_{SM} = 1.20 \pm 0.14
\end{equation}
